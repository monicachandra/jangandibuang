...
class LaporanDonasiPembeli extends StatefulWidget {
  final String tglAwal;
  final String tglAkhir;
  LaporanDonasiPembeli(
    {
...
class _MyHomePageState extends State<LaporanDonasiPembeli> {
                ...
                child: Center(
                  child:Column(
                    children:[
                      ...
                      SizedBox(height:10.0),
                      Text("Jumlah Donasi : "+myOrder.length.toString()+"x", style: TextStyle(fontSize: 22.0,fontWeight: FontWeight.bold))
                    ]
                  )
                ),
              ),
  ...
  Future<String> getListOrderan() async{
    var url = Uri.parse(globals.ipnumber+'getListOrderanPembeliDonasi');
    setState(() {
      myOrder.clear();
    });
    await http
