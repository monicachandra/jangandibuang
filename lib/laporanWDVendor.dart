...
class LaporanWDVendor extends StatefulWidget {
...
class _MyHomePageState extends State<LaporanWDVendor> {
            ...
            children: [
              ...
              Expanded(
                ...
                child: Center(
                  child:Column(
                    children:[
                      Text("Daftar Withdrawal "+globals.loginuser, style: TextStyle(fontSize: 15.0,fontWeight: FontWeight.bold),),
                      ...
                      Text("Total WD Disetujui : Rp "+myFormat.format(this.totalWithdrawal).toString(), style: TextStyle(fontSize: 20.0,fontWeight: FontWeight.bold))
                      :SizedBox(height:0)
                    ]
                  )
                ),
                      ...
                      child:DropdownButton<String>(
                        ...
                        onChanged: (value){
                          setState(() {
                            ...
                            else if(statusWD=="Menunggu Konfirmasi"){
                              statusWDint=0;
                            }
                            else if(statusWD=="Disetujui"){
                              statusWDint=1;
                            }
                            else if(statusWD=="Dibatalkan"){
                              statusWDint=2;
                            }
                        ...
                        items:<String>['Semua','Menunggu Konfirmasi','Disetujui','Dibatalkan','Ditolak'].map((String value) {
  ...
  Future<String> getWD() async{
    var url = Uri.parse(globals.ipnumber+'getLaporanWDVendor');
    setState(() {
      arrWD.clear();
    });
    await http
            .post(url, body: {'username':globals.loginuser,'tglAwal':tglAwal,'tglAkhir':tglAkhir,'status':statusWDint.toString()})
            .then((res){
